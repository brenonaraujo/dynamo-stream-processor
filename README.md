...
```bash
dynamo-stream-processor$ sam build
```
...
```bash
dynamo-stream-processor$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
dynamo-stream-processor$ sam local start-api
dynamo-stream-processor$ curl http://localhost:3000/
```
...
```bash
dynamo-stream-processor$ sam logs -n HelloWorldFunction --stack-name dynamo-stream-processor --tail
```
...
```bash
dynamo-stream-processor$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name dynamo-stream-processor
```
